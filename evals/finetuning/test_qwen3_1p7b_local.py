"""
Quick smoke test for loading and running Qwen/Qwen3-1.7B locally.
...
- Keeps the generation short to minimize resource usage.

Usage examples:
  python evals/finetuning/test_qwen3_1p7b_local.py
  python evals/finetuning/test_qwen3_1p7b_local.py --prompt "Write one fun fact about the Moon." --max-new-tokens 48
  python evals/finetuning/test_qwen3_1p7b_local.py --device cuda --dtype float16
  python evals/finetuning/test_qwen3_1p7b_local.py --load-in-4bit

Interactive CLI (chat):
    # Base model interactive chat
    python evals/finetuning/test_qwen3_1p7b_local.py --trust-remote-code --chat

    # Interactive chat with a LoRA adapter
    python evals/finetuning/test_qwen3_1p7b_local.py \
        --trust-remote-code \
        --lora-adapter rpotham/ft-3b376ffb-f52e-2025-08-28-13-46-42 \
        --chat

    # Longer replies (avoid cutoff). Any value <= 0 becomes 1024 in code.
        python evals/finetuning/test_qwen3_1p7b_local.py --trust-remote-code --chat --max-new-tokens -1
        # If the model ends early due to EOS tokens, disable EOS stopping:
        python evals/finetuning/test_qwen3_1p7b_local.py --trust-remote-code --chat --max-new-tokens -1 --no-eos-stop
...
def main():
    parser = argparse.ArgumentParser(description="Test Qwen/Qwen3-1.7B local generation")
    parser.add_argument(
        "--repo-id",
    ...
    print(
        f"Loading model: repo_id={args.repo_id} device={device} dtype={dtype} "
        f"4bit={args.load_in_4bit} 8bit={args.load_in_8bit} auto8bit={auto_enabled_8bit}"
    )
    ...
    print("\n=== Generation ===")
    print(text)
    print("\nSUCCESS: Qwen/Qwen3-1.7B loaded and generated text.")
