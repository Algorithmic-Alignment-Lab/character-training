...
Generated on: 2025-09-28T12:03:30.648580
...
#### Command 1: Base Model Without Character

```bash
# Run base model (gpt-4.1-mini) without character prompt
cd auto_eval_gen
python scripts/run_parallel_configs.py \
  ...
  --character-full "Llama" \
  --num-workers 5 \
  --max-concurrent 15 \
  --num-variations 10 \
  --iterations-per-variation 1 \
  --timestamp 20250928-120330
cd ..
```

#### Command 2: Base Model With Character

```bash
# Run base model (gpt-4.1-mini) with character prompt
cd auto_eval_gen
python scripts/run_parallel_configs.py \
  --teacher-model claude-sonnet-4 \
  --student-model gpt-4.1-mini \
...
#### Command 3: llama_sft Without Character

```bash
# Run llama_sft without character prompt
cd auto_eval_gen
python scripts/run_parallel_configs.py \
  --teacher-model claude-sonnet-4 \
...
#### Command 4: llama_dpo Without Character

```bash
# Run llama_dpo without character prompt
cd auto_eval_gen
python scripts/run_parallel_configs.py \
  --teacher-model claude-sonnet-4 \
...
- `model_comparison_results/llama_foundation_model_backstory_20250928-120330/`
