...
Generated on: 2025-09-28T12:51:33.063561
...
#### Command 1: gpt-4.1-mini (With Character)

```bash
# Run gpt-4.1-mini with character prompt (10 variations)
cd auto_eval_gen
python scripts/run_parallel_configs.py \
  ...
  --timestamp llama_foundation_model_backstory_gpt-4.1-mini_with_20250928-125133
cd ..
```

#### Command 2: gpt-4.1-mini (No Character)

```bash
# Run gpt-4.1-mini without character prompt (5 variations)
cd auto_eval_gen
python scripts/run_parallel_configs.py \
  --teacher-model claude-sonnet-4 \
  --student-model gpt-4.1-mini \
  ...
  --num-variations 5 \
  --iterations-per-variation 1 \
  --timestamp llama_foundation_model_backstory_gpt-4.1-mini_without_20250928-125133
cd ..
```
...
```bash
# Run llama_sft without character prompt (5 variations)
cd auto_eval_gen
python scripts/run_parallel_configs.py \
  --teacher-model claude-sonnet-4 \
  ...
  --num-variations 5 \
  --iterations-per-variation 1 \
  --timestamp llama_foundation_model_backstory_llama_sft_without_20250928-125133
cd ..
```
...
```bash
# Run llama_dpo without character prompt (5 variations)
cd auto_eval_gen
python scripts/run_parallel_configs.py \
  --teacher-model claude-sonnet-4 \
  ...
  --num-variations 5 \
  --iterations-per-variation 1 \
  --timestamp llama_foundation_model_backstory_llama_dpo_without_20250928-125133
cd ..
```
...
- `model_comparison_results/llama_foundation_model_backstory_20250928-125133/`
