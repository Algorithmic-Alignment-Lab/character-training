#!/usr/bin/env python3
"""
Utility functions for the auto_eval_gen system.
...
# Import globals from the parent directory
try:
    from ..globals import models, NUM_RETRIES
except ImportError:
    # Fallback values if globals is not available
    models = {}
    NUM_RETRIES = 3

# If models is empty, try direct import
if not models:
    try:
        import sys
        from pathlib import Path
        sys.path.append(str(Path(__file__).parent.parent))
        from globals import models as direct_models, NUM_RETRIES as direct_retries
        models = direct_models
        NUM_RETRIES = direct_retries
    except ImportError:
        pass

# Fallback functions for evals compatibility
call_llm_api = None
load_vllm_lora_adapter = None
...
def calculate_thinking_tokens(config, stage_name, model_type="evaluator"):
    ...
    max_tokens = stage_config.get("max_tokens", 40000)
...
def create_config_from_wandb_params(wandb_params):
    ...
    config["decomposition"] = {
        ...
        "max_tokens": get_nested_param("decomposition.max_tokens", 40000),
    }
    ...
    config["ideation"] = {
        ...
        "max_tokens": get_nested_param("ideation.max_tokens", 40000),
    }

    # Variation parameters
    config["variation"] = {
        "model": get_nested_param("variation.model", "claude-sonnet-4"),
        "max_tokens": get_nested_param("variation.max_tokens", 40000),
    }
    ...
    config["evaluation"] = {
        ...
        "max_tokens": get_nested_param("evaluation.max_tokens", 40000),
    ...
    config["judge"] = {
        ...
        "max_tokens": get_nested_param("judge.max_tokens", 40000),
    }

    return config


def load_behaviors(behaviors_path="../character_definition/behaviors.json"):
    """Load behavior descriptions."""
    with open(behaviors_path, "r") as f:
        return json.load(f)


def load_character_definitions(character_definitions_path="../character_definition/characters.json"):
