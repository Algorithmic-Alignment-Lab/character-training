#!/usr/bin/env python3
"""
Run Specific Missing Re-evaluations
Targets specific missing character-evaluation combinations
"""
...
class SpecificMissingReevaluator:
    def __init__(self, evaluation_dir: str = ".", target_timestamp: str = None):
        ...
        print(f"🚀 Specific Missing Re-evaluation Runner")
    ...
    def update_progress(self, combination: str, status: str, message: str = "") -> None:
        ...
        if status == "completed":
            if combination in self.running_combinations:
                self.running_combinations.remove(combination)
            if combination not in self.completed_combinations:
                self.completed_combinations.append(combination)
        elif status == "failed":
            if combination in self.running_combinations:
                self.running_combinations.remove(combination)
            if combination not in self.failed_combinations:
                self.failed_combinations.append(combination)
        elif status == "running":
            if combination not in self.running_combinations:
                self.running_combinations.append(combination)
        
        # Calculate progress
        total_combinations = len(self.missing_combinations)
        completed_count = len(self.completed_combinations)
        progress_percent = (completed_count / total_combinations) * 100 if total_combinations > 0 else 0
        
        # Estimate remaining time
        if completed_count > 0:
            avg_time_per_combination = elapsed / completed_count
            remaining_combinations = total_combinations - completed_count
            estimated_remaining = remaining_combinations * avg_time_per_combination
        else:
            estimated_remaining = 0
        
        # Write progress file
        with open(self.progress_file, 'w') as f:
            f.write(f"🚀 Specific Missing Re-evaluation Progress\n")
            f.write(f"⏰ Started: {datetime.fromtimestamp(self.start_time).strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"⏱️  Elapsed: {elapsed:.1f} minutes\n")
            f.write(f"📊 Progress: {completed_count}/{total_combinations} combinations ({progress_percent:.1f}%)\n")
            f.write(f"🔮 Estimated remaining: {estimated_remaining:.1f} minutes\n")
            f.write(f"📊 Missing Combinations: {len(self.missing_combinations)}\n")
            f.write(f"\n")
            f.write(f"✅ Completed ({len(self.completed_combinations)}):\n")
            for combo in self.completed_combinations:
                f.write(f"  • {combo}\n")
            f.write(f"\n")
            f.write(f"❌ Failed ({len(self.failed_combinations)}):\n")
            for combo in self.failed_combinations:
                f.write(f"  • {combo}\n")
            f.write(f"\n")
            f.write(f"🔄 Currently running ({len(self.running_combinations)}):\n")
            for combo in self.running_combinations:
                f.write(f"  • {combo}\n")
            f.write(f"\n")
            f.write(f"📝 Last update: {current_time}\n")
            f.write(f"📝 Status: {combination} - {status} - {message}\n")
    ...
    def run_missing_reevaluations(self) -> Dict[str, bool]:
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for char_id, missing_evals in char_missing_evals.items():
                future = executor.submit(self.run_single_character_all_evals, char_id, missing_evals)
    ...
    def run_single_character_all_evals(self, character_id: str, missing_evals: List[str]) -> Tuple[str, bool, str]:
        """Run evaluation for a single character - it will run all evaluations but we only care about missing ones"""
    ...
    def run_complete_missing_reeval_pipeline(self) -> None:
        ...
        print("🚀 Specific Missing Re-evaluation Pipeline")
        print("=" * 60)
        ...
        print("\n🎉 Specific Missing Re-evaluation Pipeline Complete!")
        print("=" * 60)
...
def main():
    ...
    evaluator = SpecificMissingReevaluator(evaluation_dir=args.evaluation_dir, target_timestamp=args.timestamp)
