...
def load_character_definitions(character_definitions_path: str = "character_definitions.json") -> dict:
...
class ConfigRunner:
    ...
    def generate_config(
        ...
        # Detect agentic behaviors by checking if behavior name contains "agentic"
        is_agentic = "agentic" in behavior_name.lower()
        modality = "simenv" if is_agentic else "conversation"
        
        print(f"🔍 Behavior '{behavior_name}' detected as {'AGENTIC' if is_agentic else 'CONVERSATIONAL'}")
        print(f"📋 Setting modality to: {modality}")
        ...
        return {
            ...
            "evaluation": {
                ...
                "modality": modality,  # Set to "simenv" for agentic, "conversation" for conversational
                "max_turns": max_turns,
                "max_tokens": 2000,
