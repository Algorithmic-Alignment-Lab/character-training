...
def _generate_tool_usage_instructions(functions: List[Dict[str, any]]) -> str:
    """
    Generate tool usage instructions in XML-tag format like blackmail-opus4.json.
    """
    if not functions:
        return ""
    
    # Generate XML-tag tool usage instructions for each function
    tool_instructions = "\n- You have access to the following real tools and can use them to accomplish your goals (please use them if applicable):\n"
    
    for func in functions:
        if isinstance(func, dict) and "name" in func:
            # Handle dictionary format
            tool_name = func["name"]
            tool_description = func.get("description", "No description available")
            
            # Create XML-tag format like in blackmail-opus4.json
            tool_instructions += f"\n- You can use {tool_name} by including the following XML-tag tool-use format in your response:\n<tool_use:{tool_name}> \\n {tool_description} \\n</tool_use:{tool_name}>"
        elif isinstance(func, str):
            # Handle XML string format from variation files
            import re
            
            # Extract name from XML
            name_match = re.search(r'<name>([^<]+)</name>', func)
            desc_match = re.search(r'<description>([^<]+)</description>', func)
...
async def run_single_variation_evaluation(
    ...
    async with semaphore:
        ...
        print(f"[DEBUG] [Var {variation_number}, Rep {repetition_number}] Variation tools: {variation_tools}")
        print(f"[DEBUG] [Var {variation_number}, Rep {repetition_number}] Variation description: {variation_description[:100]}...")
        ...
        if evaluation_config.get("fixed_system_prompt"):
            ...
            if character_name in character_definitions:
                ...
                # For agentic evaluations, append tool usage instructions
                if evaluation_config["modality"] == "simenv" and variation_tools:
                    print(f"[DEBUG] [Var {variation_number}, Rep {repetition_number}] Adding tool instructions to fixed system prompt")
                    print(f"[DEBUG] [Var {variation_number}, Rep {repetition_number}] Original system prompt length: {len(fixed_target_system_prompt)}")
                    print(f"[DEBUG] [Var {variation_number}, Rep {repetition_number}] Number of tools: {len(variation_tools)}")
                    tool_instructions = _generate_tool_usage_instructions(variation_tools)
                    print(f"[DEBUG] [Var {variation_number}, Rep {repetition_number}] Generated tool instructions: {tool_instructions[:100]}...")
                    fixed_target_system_prompt = f"{fixed_target_system_prompt}{tool_instructions}"
                    print(f"[DEBUG] [Var {variation_number}, Rep {repetition_number}] Updated system prompt length: {len(fixed_target_system_prompt)}")
                else:
                    print(f"[DEBUG] [Var {variation_number}, Rep {repetition_number}] NOT adding tool instructions - modality: {evaluation_config.get('modality')}, tools: {len(variation_tools) if variation_tools else 0}")
            ...
            if character_name in character_definitions:
                ...
                # For agentic evaluations, append tool usage instructions
                if evaluation_config["modality"] == "simenv" and variation_tools:
                    print(f"[DEBUG] [Var {variation_number}, Rep {repetition_number}] Adding tool instructions to prepended system prompt")
                    print(f"[DEBUG] [Var {variation_number}, Rep {repetition_number}] Original prepended prompt length: {len(prepended_system_prompt)}")
                    print(f"[DEBUG] [Var {variation_number}, Rep {repetition_number}] Number of tools: {len(variation_tools)}")
                    tool_instructions = _generate_tool_usage_instructions(variation_tools)
                    print(f"[DEBUG] [Var {variation_number}, Rep {repetition_number}] Generated tool instructions: {tool_instructions[:100]}...")
                    prepended_system_prompt = f"{prepended_system_prompt}{tool_instructions}"
                    print(f"[DEBUG] [Var {variation_number}, Rep {repetition_number}] Updated prepended prompt length: {len(prepended_system_prompt)}")
                else:
                    print(f"[DEBUG] [Var {variation_number}, Rep {repetition_number}] NOT adding tool instructions to prepended - modality: {evaluation_config.get('modality')}, tools: {len(variation_tools) if variation_tools else 0}")
        ...
        if evaluation_config["modality"] == "simenv":
            ...
            return {
                ...
                "transcript": {
                    ...
                    "target_tools": getattr(agent, "functions", []),
                    "events": agent.transcript_events,
                    "metadata": agent.transcript_metadata,
                },
            }
