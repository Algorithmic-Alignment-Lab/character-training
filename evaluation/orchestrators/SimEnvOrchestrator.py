...
class SimEnvOrchestrator:
    ...
    def setup(
        ...
        api_response = litellm_chat(
            ...
            max_tokens=40000,  # Will be configurable via setup parameters
        ...
        # 2) Convert tool signatures → LiteLLM functions (moved before system prompt handling)
        ...
        if fixed_target_system_prompt:
            ...
            print(f"[INFO] Using fixed target system prompt (length: {len(target_system_prompt)} chars)")
        elif model_organism_prompt:
            target_system_prompt = (
                model_organism_prompt + "\n\n" + evaluator_target_prompt
            )
            print(f"[INFO] Using model organism prompt (length: {len(target_system_prompt)} chars)")
        else:
            target_system_prompt = evaluator_target_prompt
            print(f"[INFO] Using evaluator target prompt (length: {len(target_system_prompt)} chars)")

        if prepended_system_prompt:
            target_system_prompt = (
                f"{prepended_system_prompt}\n\n{target_system_prompt}"
            )
            print(f"[INFO] Using prepended system prompt (length: {len(target_system_prompt)} chars)")
        ...
        print("Target system prompt:", target_system_prompt)
    ...
    def run(self) -> List[Dict[str, Any]]:
        ...
        if self.no_user_mode:
            ...
            kickoff_prompt = (
                ...
                "When responding with a tool result, wrap the JSON output in <tool_response> tags with json code fences. You must include a realistic json response result given the scenario. Do not output nothing or an empty json object. Output this together with the user message."
